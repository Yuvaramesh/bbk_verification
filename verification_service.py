...
class VerificationService:
    ...
    DOCUMENT_TYPES = ['license', 'id', 'proof_of_address', 'bank_statement', 'utility_bill', 'passport']
    ...
    def verify_with_firmcheck(document_id, document_data):
        ...
        verification_result = FirmCheckService.verify_document(
            document_data.get('extracted_fields', document_data),
            document_type
        )
        ...
        documents.update_one(
            {'_id': ObjectId(document_id)},
            {
                '$set': {
                    ...
                    'verification_score': FirmCheckService.get_verification_score(verification_result),
    ...
    def get_today_stats():
        ...
        stats = list(verifications.aggregate(pipeline))
        
        total = sum(s['count'] for s in stats)
        pending = next((s['count'] for s in stats if s['_id'] == 'pending'), 0)
        under_review = next((s['count'] for s in stats if s['_id'] == 'under_review'), 0)
        approved = next((s['count'] for s in stats if s['_id'] == 'approved'), 0)
        
        admin_count = verifications.count_documents({
            'admin_approved': True,
            'created_at': {'$gte': today_start, '$lt': today_end}
        })
        
        ceo_count = verifications.count_documents({
            'ceo_approved': True,
            'created_at': {'$gte': today_start, '$lt': today_end}
        })
        
        cleared_count = verifications.count_documents({
            'cleared': True,
            'created_at': {'$gte': today_start, '$lt': today_end}
        })
        ...
        return {
            ...
            'approved': approved,
            'admin_approved': admin_count,
            'ceo_approved': ceo_count,
